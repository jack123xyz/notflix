<template>
  <div class="-mt-2 -mb-3 lg:mb-1">
    ...
    <div class="flex items-center justify-between mb-6 mx-8">
      ...
      <div
        :id="`pagination-${carID}`"
        class="pagination-container flex items-center hidden md:flex"
      ></div>
    </div>
        ...
        <template v-if="!isLoading">
          ...
          >
            ...
            <div
              ...
              :class="{ 'tenth-item-container': movie.ranking === 10 }"
            >
              <!-- Ranking Number with simplified classes -->
              <span
                class="ranking-number"
                :class="{
                  'number-10': movie.ranking === 10,
                }"
                >{{ movie.ranking }}</span
              >

              <!-- Movie poster with simplified sizing -->
              <div
                class="poster-container"
                :class="{ 'tenth-poster': movie.ranking === 10 }"
                ref="movieRefs"
            ...
            <!-- Movie hover card (teleported to body) -->
            <teleport to="body">
              <MovieHoverCard
...
const props = defineProps({
  title: {
    type: String,
    default: "Top 10 Movies in USA Today",
  },
  movies: Array,
  isLoading: Boolean,
  logos: Object,
...
function initSwiper() {
  ...
  if (!sw || !props.movies?.length) return;

  try {
    setActiveBreakpoint();
    const viewportWidthInRem = window.innerWidth / 16;

    // Configure touch settings based on isMobileView ref
    ...
    // Ensure we always show all 10 movies if possible
    ...
    sw.params = {
      ...
      slidesPerView: isMobileView.value
        ? Math.min(4, activeBreakpoint.value.slidesPerView) // Use smaller value for mobile
        : Math.max(
            activeBreakpoint.value.slidesPerView,
      ...
      allowTouchMove: isMobileView.value, // Use isMobileView ref
    };
    ...
    console.log("TopTenMovieGrid: Touch enabled:", isMobileView.value);
  } catch (error) {
...
function startHoverTimer(movieId) {
  ...
  setTimeout(() => {
    ...
    if (element) {
      ...
      // Find the movie's ranking
      const movie = props.movies.find((m) => m.id === movieId);
      const isTenthMovie = movie?.ranking === 10;
      ...
      // Adjust for 10th movie if needed
      if (isTenthMovie) {
